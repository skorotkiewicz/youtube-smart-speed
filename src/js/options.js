document.addEventListener("DOMContentLoaded", () => {
	// Get defaults from background script to avoid duplication
	chrome.runtime.sendMessage({ action: "getDefaults" }, (response) => {
		...
		// Supported video service URLs (no duplication)
		const supportedUrls = [
			"*://*.youtube.com/*",
			"*://youtube.com/*",
			"*://*.vimeo.com/*",
		...
		document.getElementById("save").addEventListener("click", () => {
			...
			chrome.storage.sync.set(settings, () => {
				...
				// Update all supported video service tabs
				supportedUrls.forEach((urlPattern) => {
		...
		document.getElementById("resetModel").addEventListener("click", () => {
			if (
				...
				chrome.storage.local.remove("smartSpeedModel", () => {
					...
					// Reset model on all supported video service tabs
					supportedUrls.forEach((urlPattern) => {
						chrome.tabs.query({ url: urlPattern }, (tabs) => {
							tabs.forEach((tab) => {
